...
static int m,n;
static int line[501][501],boy[501],book[501];
//line数组标记可能的组合
//boy数组记录匹配的伴侣
//book数组作为标记,记录每一轮查找时,选择对象的占用情况---(在下方代码中代入题意理解)
//--->如果可能组合的book数组都被占用,表示无法成功匹配
bool find(int x){      //查找是否有男伴侣 
	int i;
	for(i = 1;i <= m;i++){  //查找每一名对象
		if(line[x][i] == 1 && book[i] == 0){    //存在没有被选择的可能伴侣
			book[i]=1;      //在这轮遍历中标记该名对象为伴侣
...
int main()
{
	...
	while(cin>>t>>n>>m,t){
		...
		int sum=0;
		for(i = 1;i <= n;i++){
			memset(book,0,sizeof(book));    //对每一次查找，都要重置
